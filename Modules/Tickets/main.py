...
from Config.Tickets.config import *

######## editable parameters
data_file_name = save_path + "/data.json"
########
...
class Tickets(commands.Cog):
    ...
    async def create_ticket(self, ctx):
        ...
        data = await Tickets.dataExists(ctx, True)
        if data is None :
            return
    ...
    async def ticket_help(self, ctx):
        data = await Tickets.dataExists(ctx)
        if  data is None :
            return
    ...
    async def close(self, ctx):
        ...
        if ctx.channel.id in data["ticket-channel-ids"]:
            ...
            def check(message):
                return message.author == ctx.author and message.content.lower() == "close"
            try:
                ...
                for user in ctx.channel.members:
                    if not user.bot:
                        ...
                        await Tickets.SendLog(member, f"Closed Ticked: Id {ticket_name}", 0xF42069, file=fileObject)   
